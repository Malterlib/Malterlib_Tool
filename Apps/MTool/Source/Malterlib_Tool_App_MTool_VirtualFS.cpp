...
struct CCOFFObject
{
	...
	struct CFile
	{
		...
		void f_AddSymbol(CStr const &_Name, TCVector<uint8> const &_Data, CSection *_pSection)
		{
	...
	static TCVector<uint8> fs_GenerateData(CStr const &_SymbolName, TCVector<uint8> const &_Data)
	{
		CStr SymbolName;
...
public:
	...
	{		
		...
		CStr Platform = "OSX";
		NBuild::CMalterlibDependencyTracker MalterlibDependencyTracker;
...
//					DependencyContents += CStr::CFormat(" \\\n  {}") << DependDir;
		...
		{
			TCVector<uint8> lData = TargetMemoryStream.f_MoveVector();
